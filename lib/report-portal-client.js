...
class RPClient {
    ...
    isLaunchWithFailedTests(launchTempId) {
        ...
        return this.restClient.retrieve(launch_latest_url).then((response) => {
            this.logDebug(`Launch ID${response.content[0].id}`);
            const url = `/launch?filter.eq.id=${
                response.content[0].id
            }&filter.eq.statistics%24executions%24failed=0`;
            return this.restClient.retrieve(url).then((response) => {
                this.logDebug(
                    `Failed test elements: ${response.page.totalElements}`,
                );
    ...
    async deleteSkippedTest() {
        ...
        for (const item in launchIds) {
            await this.restClient
                .retrieve(
                    `/item?filter.eq.launchId=${
                        launchIds[item]
                    }&page.size=100`,
                )
                .then((response) => {
                    this.logDebug(
                        `Total elements in ${launchIds[item]}: ${
                            ...
                            this.restClient
                                ...
                                .then((response) => {
                                    ...
                                    this.restClient
                                        ...
                                        .then((response) => {
                                            this.logDebug(
                                                "Deleted Skipped tests successfully",
                                            );
                                        });
                                });
                        }
    ...
    async mergeLaunches(mergeDescription) {
        if (this.isLaunchMergeRequired) {
            ...
            const data = this.getMergeLaunchesData(
                activeLaunches,
                mergeDescription,
            );
            this.restClient
    ...
    sendLogWithFile(itemTempId, saveLogRQ, fileObj) {
        ...
        // eslint-disable-next-line max-len
        const requestPromise = (itemUuid, launchUuid) => this.getRequestLogWithFile(
            Object.assign(saveLogRQ, { itemUuid, launchUuid }),
            fileObj,
        );
