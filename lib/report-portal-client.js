...
findAndDeleteSkippedTests(launchTempId) {
    const launchObj = this.map[launchTempId];
    const launch_latest_url = "/launch/latest?filter.eq.uuid="+launchObj.realId;
    ...
    .then(response => {
        this.logDebug('Launch ID :'+response.content[0].id)
        const url = "/item?filter.eq.launchId="+response.content[0].id+"&isLatest=false"
        return this.restClient
            .retrieve(url)
            .then(response => {
                this.logDebug  (`Total elements`+response.page.totalElements);
                for (var i=1;i<response.page.totalElements;i++){
                    this.logDebug('Test ID: '+response.content[i].id)
                    this.logDebug('Test ID: '+response.content[i].name)
                    this.logDebug('Skipped Test: '+response.content[i].statistics.executions.skipped)
                    ...
                    if (response.content[i].statistics.executions.skipped){
                        ...
                        .then(response => {
                            ...
                            console.log(test_item_delete_url)
                            return this.restClient
